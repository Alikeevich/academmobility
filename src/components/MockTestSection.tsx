...
import { Clock, ChevronLeft, ChevronRight, HelpCircle, CheckCircle2, Play, Pause, Headphones, BookOpen, PenTool, Loader2 } from 'lucide-react';
  ...
  const handleAiGrading = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        TASK:
        Evaluate the following IELTS Writing Task 2 essay based on official criteria.
        ...
        OUTPUT FORMAT:
        Give a formal evaluation in HTML format (use only basic tags like <b>, <br>, <ul>, <li>, <p>):
      ...
      setAiFeedback("<p style='color:red'>System is currently busy. Assessment could not be completed at this time.</p>");
    } finally {
      setIsGrading(false);
    }
  };
      ...
      <section id="mock-test" className="py-24 bg-uni-gray">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-600">Практикуйтесь в реальных условиях экзамена.</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            {writingText.length > 0 && (
              <div className="bg-yellow-50 p-8 rounded-xl border border-yellow-200 shadow-inner">
                <div className="mb-4 border-b border-yellow-200 pb-2 flex justify-between items-center">
                  <h3 className="font-bold text-lg text-yellow-800 flex items-center gap-2">
                    <PenTool className="w-5 h-5" /> Writing Assessment
                  </h3>
                  <span className="text-sm text-gray-500">Word Count: {wWords}</span>
                </div>
                ...
                {isGrading && (
                  <div className="flex flex-col items-center justify-center py-10 text-yellow-800">
                    ...
                    <p className="font-bold text-lg">Calculating Band Score...</p>
                    <p className="text-sm text-gray-500">Analyzing Task Response and Coherence</p>
                  </div>
                )}

                {/* Feedback Result (AUTOMATIC) */}
                {!isGrading && aiFeedback && (
                  <div className="bg-white p-6 rounded-lg border border-yellow-100 text-gray-800 animate-in fade-in duration-500">
                ...
                {!isGrading && !aiFeedback && writingText.length <= 50 && (
                   <p className="text-red-500 text-center italic">Essay is too short to be assessed (Minimum 50 words).</p>
                )}
              </div>
            )}
          </div>
